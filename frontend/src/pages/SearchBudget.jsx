...
export function SearchBudget() {
  ...
  const renderContent = () => {
    if (budgets.length === 0) {
      return (
        <span className=" flex gap-1 items-center text-zinc-100 w-72 bg-slate-900 border border-orange-500 p-5 rounded-sm">
    ...
    return(
      <div className=" flex flex-col items-center justify-center mt-10 w-full gap-8 mb-10">
        ...
        <div className="flex flex-wrap flex-col sm:flex-row gap-5 w-full items-center justify-center">
          <input type="text" placeholder="Nome do cliente" value={clientName} onChange={e => setClientName(e.target.value)} className="border border-orange-500 text-sm bg-transparent ease-in duration-150 text-zinc-50 rounded p-1" />
          <input type="text" placeholder="Nome do vendedor" value={sellerName} onChange={e => setSellerName(e.target.value)} className="border border-orange-500 text-sm  bg-transparent ease-in duration-150 text-zinc-50 rounded p-1" />
          <div className="flex flex-col sm:flex-row  gap-1 items-center">
            <label className="text-zinc-100 text-sm">Data inicial</label>
          <DateTimePicker
            className= ' w-full border border-orange-500 bg-transparent ease-in duration-150 text-zinc-50 rounded p-1'
            format="dd-MM-yyyy HH:mm"
            value={startDate}
            onChange={date => setStartDate(date)}
            locale="pt-BR"
            calendarIcon={<CalendarOutlined className="hover:text-orange-500 text-sm" />}
            calendarClassName="bg-slate-950"
            clearIcon={<CloseOutlined className="hover:text-orange-500 text-sm" />}
            disableClock={true}
          />
          </div>
        <div className="flex flex-col sm:flex-row gap-1 items-center">
          <label className="text-zinc-100 text-sm">Data final</label>
        <DateTimePicker
            className= ' w-full border border-orange-500 bg-transparent ease-in duration-150 text-zinc-50 rounded p-1'
            format="dd-MM-yyyy HH:mm"
            value={endDate}
          ...
          <button onClick={fetchBudgets} className="border text-sm  border-orange-500 ease-in duration-150 text-zinc-50 rounded p-1 hover:bg-orange-500">Pesquisar</button>
        </div>
          ...
          {budgets.map(budget =>{
            return(
              <li key={budget.id} className=" flex flex-col bg-slate-900 border border-orange-500 rounded-sm p-5 gap-2 w-11/12 sm:w-2/5 break-words relative">
                ...
                <span className="bg-orange-500 flex gap-2 items-center outline-none border-none break-words rounded-sm p-1">
                  <UserOutlined />
